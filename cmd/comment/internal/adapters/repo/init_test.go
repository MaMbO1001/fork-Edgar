...
import (
	...
	"github.com/ZergsLaw/back-template/cmd/comment/internal/adapters/repo"
...
func start(t *testing.T) (context.Context, *repo.Jopa, *require.Assertions) {
	t.Helper()
	...
	j, err := repo.New(ctx, reg, namespace, repo.Config{
	...
	return logger.NewContext(ctx, log), j, assert
}
